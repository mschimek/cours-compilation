...
n_instr *instruction(void) 
{
	...
	else if (est_premier(_instructionEcriturec_, uniteCourante)) {
		instr = instructionEcriturec();
		goto end;
	}
...
n_instr *instructionEcriturec(void) 
{
	n_exp * expr = NULL;
	//affiche_balise_ouvrante__func__, TRACE_XML);
	if (uniteCourante == ECRIREC) {
		afficher_sans_afficher();
		uniteCourante = yylex();
		if (uniteCourante == PARENTHESE_OUVRANTE) {
			afficher_sans_afficher();
	...
	//affiche_balise_fermante(__func__, TRACE_XML);
	return cree_n_instr_ecrirec(expr);
}
