...
char *tableMotsClefs[] = {
  "si", "alors", "sinon", "tantque", "faire", "entier", "retour", "lire", "ecrire", "ecrirec"
};
...
int codeSymbolesSimples[] = { 
  ...
  CROCHET_OUVRANT, CROCHET_FERMANT, ACCOLADE_OUVRANTE, ACCOLADE_FERMANTE, EGAL, 
...
int estSymboleSimple(char c) {
  for (int i = 0; i < sizeof(tableSymbolesSimples); i++) {
    if (c == tableSymbolesSimples[i])
